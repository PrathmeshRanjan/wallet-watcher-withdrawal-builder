  ...
  return (
    ...
    >
      <div className="wallet-card-top">
        <span className="wallet-index">
          W{String(wallet.index + 1).padStart(2, "0")}
        </span>
        <span className="wallet-path">
          {wallet.derivationPath.split("/").at(-1)}
        </span>
      </div>
...
function ChangeIcon({ change }: { change: BalanceChange }) {
  if (change.kind === "INFLOW" || change.kind === "INITIAL_BALANCE")
...
function ActivityFeed({ walletIndex }: { walletIndex: number }) {
  ...
  if (changes.isLoading)
    return <div className="panel-empty">Loading activity…</div>;
      ...
      {changes.data.changes.map((change) => {
        ...
        return (
          <div className="activity-item" key={change.id}>
            <div
              className={`activity-icon ${positive ? "positive" : "negative"}`}
            >
            ...
            <div
              className={`activity-amount ${positive ? "positive" : "negative"}`}
            >
              {positive ? "+" : ""}
              {displayEth(change.delta.eth)} ETH
            </div>
          </div>
        );
      })}
          ...
          <div className="result-view">
            <div className={`result-status ${result.state.toLowerCase()}`}>
              ...
              <div>
                ...
                <span>
                  {result.amount.eth} ETH to {shortAddress(result.to)}
                </span>
              </div>
            </div>
            <div className="payload-section">
              <div className="payload-title">
                ...
                <CopyButton value={result.txHash} />
              </div>
            ...
            {result.signedTransaction && (
              <div className="payload-section">
                <div className="payload-title">
                  <span>Signed raw transaction</span>
                  <CopyButton value={result.signedTransaction} />
                </div>
            ...
            <div className="signature-grid">
              <div>
                <span>Nonce</span>
                <strong>{result.transaction.nonce}</strong>
              </div>
              <div>
                <span>Gas limit</span>
                <strong>{result.transaction.gasLimit}</strong>
              </div>
              <div>
                <span>Signature parity</span>
                <strong>{result.signature.yParity}</strong>
              </div>
            </div>
            {result.state === "BUILT" ? (
          ...
          <form onSubmit={submit}>
            <div className="source-summary">
              ...
              <div className="source-balance">
                <span>Available</span>
                <strong>{displayEth(wallet.balance.eth)} ETH</strong>
              </div>
            </div>
            <label>
              Destination address
            ...
            <div className="mode-selector">
              <button
                type="button"
                className={!broadcast ? "active" : ""}
                onClick={() => setBroadcast(false)}
              >
                <Eye size={17} />
                <span>
                  <strong>Build only</strong>
                  <small>Sign without sending</small>
                </span>
              </button>
              <button
                type="button"
                className={broadcast ? "active danger" : ""}
                onClick={() => setBroadcast(true)}
              >
                <Send size={17} />
                <span>
                  <strong>Build & broadcast</strong>
                  <small>Send on Base Sepolia</small>
                </span>
              </button>
            </div>
            <div className="safety-note">
              <ShieldCheck size={17} />
              <span>
                Testnet only. The service reserves enough balance for the
                maximum estimated network fee.
              </span>
            </div>
            {mutation.error && (
            ...
            <button
              className={`primary-button ${broadcast ? "broadcast" : ""}`}
...
function WithdrawalList({ withdrawals }: { withdrawals: Withdrawal[] }) {
  if (!withdrawals.length)
    return (
      <div className="panel-empty">
        <Send size={22} />
        <span>No withdrawals built yet</span>
      </div>
    );
  return (
    <div className="withdrawal-list">
      {withdrawals.slice(0, 6).map((item) => (
        <div key={item.id} className="withdrawal-item">
          <div className="withdrawal-arrow">
            <ArrowUpRight size={16} />
          </div>
          <div className="withdrawal-copy">
            <strong>{item.amount.eth} ETH</strong>
            <span>{shortAddress(item.to)}</span>
          </div>
          <span className={`status-pill ${item.state.toLowerCase()}`}>
            {item.state}
          </span>
          {item.state !== "BUILT" && (
            <a
...
export function App() {
  ...
  const selectedWallet =
    wallets.data?.wallets.find((wallet) => wallet.index === selectedIndex) ??
    wallets.data?.wallets[0];
  const total = useMemo(
    () =>
      wallets.data?.wallets.reduce(
        (sum, wallet) => sum + Number(wallet.balance.eth),
        0,
      ) ?? 0,
    [wallets.data],
  );
  ...
  if (wallets.error || !wallets.data) {
    return (
      <main className="loading-screen error-screen">
        <CircleAlert />
        <h1>Couldn’t reach the wallet service</h1>
        <p>{wallets.error?.message}</p>
        <button className="primary-button" onClick={() => wallets.refetch()}>
          Try again
        </button>
      </main>
    );
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <div className={`network-pill ${wallets.data.stale ? "stale" : ""}`}>
            <span className="network-dot" />
            Base Sepolia <small>84532</small>
          </div>
          <button
      ...
      <section className="hero">
        <div>
          <span className="eyebrow">
            <Sparkles size={13} /> Testnet custody workspace
          </span>
          <h1>
            Every wallet.
            <br />
          ...
          <p>
            Deterministic accounts, block-pinned balances, and controlled
        ...
        <div className="hero-stats">
          ...
          <div>
            ...
            <strong>{wallets.data.wallets.length}</strong>
            <small>active</small>
          </div>
          <div>
            <span>Last reconciliation</span>
            <strong className="stat-time">
              {relativeTime(wallets.data.lastSuccessAt)}
            </strong>
            <small>{wallets.data.stale ? "stale" : "healthy"}</small>
          </div>
        </div>
      </section>
        ...
        <div className="wallets-section">
          <div className="section-heading">
            ...
            <span className="section-count">
              {wallets.data.wallets.length} / 20
            </span>
          </div>
          <div className="wallet-grid">
            {wallets.data.wallets.map((wallet) => (
              <WalletCard
                key={wallet.index}
                wallet={wallet}
                selected={selectedWallet?.index === wallet.index}
                onSelect={() => setSelectedIndex(wallet.index)}
              />
            ))}
          </div>
        </div>
          ...
          <aside className="wallet-detail">
            <div className="detail-header">
              <div>
                ...
                <h2>Wallet {selectedWallet.index + 1}</h2>
              </div>
              <a
                className="icon-button"
            ...
            <div className="detail-balance">
              ...
              <div>
                <strong>{displayEth(selectedWallet.balance.eth)}</strong>
                <em>ETH</em>
              </div>
            </div>
            <div className="detail-address">
              <span>Address</span>
              <div>
                <code>{shortAddress(selectedWallet.address)}</code>
            ...
            <div className="detail-metadata">
              <div>
                ...
                <strong>{selectedWallet.derivationPath}</strong>
              </div>
              <div>
                <span>Observed block</span>
                <strong>
                  {selectedWallet.blockNumber?.toLocaleString() ?? "Pending"}
                </strong>
              </div>
            </div>
            <button
      ...
      {selectedWallet && (
        <section className="lower-grid">
          <div className="panel">
            ...
            <ActivityFeed walletIndex={selectedWallet.index} />
          </div>
          <div className="panel">
            <div className="panel-header">
              <div>
            ...
            <WithdrawalList withdrawals={withdrawals.data?.withdrawals ?? []} />
          </div>
        </section>
      )}
